...
void Solver_MCSVM_CS::solve_sub_problem(float A_i, int yi, float C_yi, int active_i, float *alpha_new)
{
    ...
    if(yi < active_i)
        D[yi] += A_i*C_yi;
    qsort(D, active_i, sizeof(float), compare_float);
                               ...
                               float Cp, float Cn, int solver_type)
{
    ...
    float diag[3] = {0.5f/Cn, 0.0f, 0.5f/Cp};
    ...
    while (iter < max_iter)
    {
        ...
        for (s=0; s<active_size; s++)
        {
            ...
            if(fabs(PG) > 1.0e-12)
            {
                ...
                alpha[i] = fmin(fmax(alpha[i] - G/QD[i], 0.0), C);
...
void solve_l2r_lr_dual(const problem *prob, float *w, float eps, float Cp, float Cn)
{
    ...
    float innereps = 1e-2;
    float innereps_min = fmin(1e-8, eps);
    ...
    while (iter < max_iter)
    {
        ...
        if(newton_iter <= l/10)
            innereps = fmax(innereps_min, 0.1*innereps);
...
float predict(const model *model_, const feature_node *x)
{
    ...
    float *w=model_->w;
    float result = 0.0;
    for(; (idx=lx->index)!=-1 && idx < model_->nr_feature; lx++){
        result += w[idx-1]; //为了这儿减少一次减法，在getglobalfeature的地方改了index的初值为0;
    }
    return result;
...
struct model *load_model_bin(std::ifstream& fin)
{
    ...
    fin.read((char*)&model_->bias, sizeof(float));
