...
public:
	...
	std::vector<cv::Mat_<float> > Train(std::vector<cv::Mat_<cv::Vec3b> >& images,
    ...
    struct feature_node* GetGlobalBinaryFeatures(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, cv::Mat_<float>& rotation, float scale, float& score, float& variance, int& is_face);
    struct feature_node* NegMineGetGlobalBinaryFeatures(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, cv::Mat_<float>& rotation, float scale, float& score, int& is_face, int stage, int currentStage, int landmark, int tree, bool& stop);
	cv::Mat_<float> Predict(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape,
		BoundingBox& bbox, cv::Mat_<float>& rotation, float scale, float& score, float& variance, int& is_face);
    cv::Mat_<float> NegMinePredict(cv::Mat_<cv::Vec3b>& image,
...
//    void ConstructLeafCount();
    // struct feature_node* GetGlobalBinaryFeaturesThread(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, cv::Mat_<float>& rotation, float scale);
    struct feature_node* GetGlobalBinaryFeaturesMP(cv::Mat_<cv::Vec3b>& image,
...
public:
	Parameters params_;
	std::vector<cv::Mat_<cv::Vec3b> > images_;
...
public:
	CascadeRegressor();
	void Train(std::vector<cv::Mat_<cv::Vec3b> >& images,
		std::vector<cv::Mat_<float> >& ground_truth_shapes,
        std::vector<int> ground_truth_faces,
		std::vector<BoundingBox>& bboxes,
		Parameters& params,
        int pos_num);
	cv::Mat_<float> Predict(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, cv::Mat_<float>& ground_truth_shape);
	cv::Mat_<float> Predict(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, int& is_face, float& score, float& variance);
    cv::Mat_<float> NegMinePredict(cv::Mat_<cv::Vec3b>& image,
    ...
    std::vector<cv::Rect> detectMultiScale(cv::Mat_<cv::Vec3b>& image,
