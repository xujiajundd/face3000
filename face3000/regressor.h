...
public:
    ...
    struct feature_node* GetGlobalBinaryFeatures(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, cv::Mat_<float>& rotation, float scale, float& score, int& is_face);
...
public:
	...
	cv::Mat_<float> Predict(cv::Mat_<cv::Vec3b>& image, cv::Mat_<float>& current_shape, BoundingBox& bbox, int& is_face, float &score);
    cv::Mat_<float> NegMinePredict(cv::Mat_<cv::Vec3b>& image,
